...
SELECT t.*,veh_start + (veh_end-veh_start)/2 AS BRK_START, veh_start + (veh_end-veh_start)/2+t.BRK_DURATION AS BRK_END FROM (
    ...
    WHERE  s.travel_date = '25-DEC-2034' AND s.DISPOSITION ='T' AND s.ALIAS_START = a.ALIAS AND s.ALIAS_END = b.alias
...
where travel_date = '25-DEC-2034' AND t.DISPOSITION ='T' AND t.ALIAS_S = a.ALIAS AND t.ALIAS_E = b.alias
AND trip_type <> 'BRK'
...
async function getData_segments(){
	var connection = await oracledb.getConnection({
	  user: 'RTD',
	  password: 'RTD',
	  connectString: '(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.85.121)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=RTD21PDB)))'
	});
